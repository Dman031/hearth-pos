...
Brand mark hierarchy (Derrick, 2026-07-15): **face-orb = hero, signet = tiny icon only.** Hero screens (splash/auth/entity-setup/onboarding) render the face-orb PNG via `src/components/Crest.tsx` (static, explicit dimensions — native art is 804×824; `ink` variant on paper surfaces, `light` on dark). Header renders `assets/brand/lockup-horizontal-header.png` (content-trimmed derivative of `lockup-horizontal.png`). The ring/dot signet reads as a bullseye at size and lives ONLY at small scale: app icon, adaptive icons, favicon (generated from `src/constants/brand.ts`). HearthOrb was REMOVED 2026-07-15 (recipe in docs/HARVESTONCE_ORB_RECIPE.md is history only — do not resurrect without Derrick). DEFERRED(brand-vector): hero/header marks are PNG for now; vector/hi-res pass before Day 30.
