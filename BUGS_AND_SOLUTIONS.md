...
**THE FIX IS NARROWER THAN THE CLASS, AND THAT IS STATED HERE SO IT IS NOT MISTAKEN FOR CLOSED** (ruling, Derrick 2026-09-10). What was repaired is ONE predicate. The class is **absence read as evidence of a specific past event** — a null, an empty set, or a missing row taken to mean "the thing happened and then stopped" when it equally means "the thing never started". `isTimeLetGo` read a null hold as *a hold that lapsed*; it was also *a hold that was never taken*. Nothing distinguishes those two readings except an enumeration of writers, and nothing enforces that enumeration.

**NOBODY HAS SWEPT FOR SIBLINGS.** The grep above is a starting point and not the sweep: it finds explicit null comparisons in two directories, and the class also lives in `?? fallback` defaults, `.length === 0` branches, `!found` arms in SQL, and `maybeSingle()` results — in both repos, and in the Worker as readily as the app. A sibling in SQL or in a tool handler would not appear in any grep written above.
...
**Promoted to CLAUDE.md rule on 2026-09-11** — the CROSS-REPO CITATION RULE, with its own grep, scope carve-outs (applied migrations and this ledger are excluded and why), and the one-time sweep of all EIGHT sites in the same commit. **This entry's own count of six was wrong**, and the promotion sweep is what found it: running the rule's grep — after the six were already fixed — turned up two more citations that narrower greps had reported clean (`practice.ts` → `reach-entity.ts:393`, `visit-copy.ts` → `medplum.ts:28-32`, both landing but both line numbers). It also found two more STALE ones this entry had missed: `visit-copy.ts`'s skipped-reason count (pointing at prose, a function parameter and a `pdfBase64` type field) and **CLAUDE.md's own token-plane citation** (`handler.ts:185-203`, now a URL builder) — the instruction file was violating the rule two sections above where the rule now lives. **Final tally: EIGHT citations, FOUR stale**, and every symbol-or-quote citation correct. The lesson is in the rule: the sweep is the command, not an approximation of it.
