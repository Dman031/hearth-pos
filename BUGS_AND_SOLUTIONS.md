...
**Promoted to CLAUDE.md rule on 2026-09-11** — the CROSS-REPO CITATION RULE, with its own grep, scope carve-outs (applied migrations and this ledger are excluded and why), and the one-time sweep of all six sites in the same commit. The promotion sweep found **two more stale citations this entry had not**: `visit-copy.ts`'s skipped-reason count (pointing at prose, a function parameter and a `pdfBase64` type field) and **CLAUDE.md's own token-plane citation** (`handler.ts:185-203`, now a URL builder) — the instruction file was violating the rule two sections above where the rule now lives. Final tally: four of six stale, and every symbol-or-quote citation correct.
